...
for (i in 1:nrow(x)) {
     ...
     if (is.na(x$AirTC_Avg[i])) {
          ...
          if (i > 4) {
               if ( (x$AirTC_Avg[i] < (mean(x$AirTC_Avg[(i-4):(i-1)]) - 2 * ( mean(x$AirTC_Max[(i-4):(i-1)]) - mean(x$AirTC_Min[(i-4):(i-1)]) ) )) | (x$AirTC_Avg[i] > (mean(x$AirTC_Avg[(i-4):(i-1)]) + 2 * ( mean(x$AirTC_Max[(i-4):(i-1)]) - mean(x$AirTC_Min[(i-4):(i-1)]) ) )) ) {
...
rain <- z %>% 
     ...
     select(unix_utc,Value,time_et,utc_offset,time_utc,site,Variable,source,qc) %>% 
     mutate(method = "Rain")
level1 <- rbind(batt,air_avg,air_min,air_max,air_std,rh_min,rh_max,ws_avg,ws_min,ws_max,ws_std,wdir,wdir_sd,rain)
rm(batt,air_avg,air_min,air_max,air_std,rh_min,rh_max,ws_avg,ws_min,ws_max,ws_std,wdir,wdir_sd,rain)

level1 <- level1[order(level1$unix_utc),]
level1 <- level1[,c(2,3,4,5,6,7,10,8,9)]
level1 <- level1 %>% 
     mutate(time_utc = as.character(time_utc)) %>% 
     mutate(time_et = as.character(time_et)) # Should now have 9 columns/variables

write_csv(level1, "/Users/davidkahler/Documents/Wind_Turbines/DuqMellon_HIS.csv", append = TRUE)
